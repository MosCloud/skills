...
解析完成后，先尝试加载 `profiling-analysis` skill 进行专业化分析。

### 6.1 尝试使用 profiling-analysis skill

```python
skill("profiling-analysis")
```

- **skill 加载成功**：将解析后的数据库路径告知用户，然后完全遵循 `profiling-analysis` 的工作流程进行分析。该 skill 会分阶段引导：需求确认 → 算子映射 → 全流扫描 → 定向钻取 → 报告，每步通过 `question` 工具与用户交互确认。
- **skill 未找到**：使用下方的独立分析流程。

### 6.2 独立分析（fallback）

如果 `profiling-analysis` 不可用，使用以下方法：

1. **流分布** — 查看 NPU 流有哪些，各自的 task 数量、wall time、compute time。
   ```sql
   SELECT streamId, COUNT(*) FROM TASK GROUP BY streamId ORDER BY COUNT(*) DESC
   ```

2. **算子分布** — 按算子名称在各流上的分布计数。
   ```sql
   SELECT t.streamId, s.value, COUNT(*) FROM TASK t
...
3. **流同步** — 统计 `aclrtStreamWaitEvent` 调用次数。
